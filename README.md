# 2026世界杯赛程日历 · 2026世界盃賽程日曆 · 2026 FIFA World Cup Calendar

[简体中文](#简体中文) | [繁體中文](#繁體中文) | [English](#english)

2026 年美加墨世界杯完整赛程，ICS 日历格式。支持两种导入方式——**日历订阅（推荐）** 可自动同步后续更新，**下载导入** 适合一次性添加。

---

## 简体中文

[:arrow_up: 回到顶部](#2026世界杯赛程日历--2026世界盃賽程日曆--2026-fifa-world-cup-calendar) | [繁體中文](#繁體中文) | [English](#english)

### 概述

- **赛事**：2026 年美加墨世界杯
- **日期**：2026 年 6 月 11 日 – 7 月 19 日
- **参赛队伍**：48 支（首次扩军）
- **总场次**：104 场
- **赛制**：12 个小组 → 32 强 → 16 强 → 8 强 → 半决赛 → 决赛

### 方式一：日历订阅（推荐）

**优点**：后续淘汰赛对阵更新后，所有订阅者自动同步，无需重新导入。

**iOS / macOS 用户**：在 Safari 中点击下方链接，自动唤起日历 App 订阅。

| 语言 | 一键订阅（Safari 点击） | 手动订阅链接（复制粘贴） |
|------|----------------------|------------------------|
| 简体中文 | <a href="webcal://ztow.github.io/worldcup-2026-calendar/2026%E4%B8%96%E7%95%8C%E6%9D%AF%E8%B5%9B%E7%A8%8B_%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87.ics">点击订阅</a> | `https://ztow.github.io/worldcup-2026-calendar/2026世界杯赛程_简体中文.ics` |
| 繁體中文 | <a href="webcal://ztow.github.io/worldcup-2026-calendar/2026%E4%B8%96%E7%95%8C%E7%9B%83%E8%B3%BD%E7%A8%8B_%E7%B9%81%E9%AB%94%E4%B8%AD%E6%96%87.ics">點擊訂閱</a> | `https://ztow.github.io/worldcup-2026-calendar/2026世界盃賽程_繁體中文.ics` |
| English | <a href="webcal://ztow.github.io/worldcup-2026-calendar/2026_FIFA_World_Cup_Schedule_EN.ics">Subscribe</a> | `https://ztow.github.io/worldcup-2026-calendar/2026_FIFA_World_Cup_Schedule_EN.ics` |

> 如果一键订阅无反应，请用 Safari 打开此页面重试，或复制右侧链接手动添加到日历。

#### 各平台详细步骤

**iPhone / iPad（iOS）**

1. 用 **Safari** 打开本页面，点击上方的「点击订阅」
2. 或：设置 → App → 日历 → 账户 → 添加账户 → 其他 → 添加已订阅的日历 → 粘贴 https 链接

**Mac（macOS）**

1. 日历 App → 菜单栏：文件 → 新建日历订阅 → 粘贴 https 链接 → 订阅

**Android**

1. 用手机浏览器打开 [Google 日历网页版](https://calendar.google.com)（需桌面模式）
2. 左侧「其他日历」→ `+` → 通过网址添加 → 粘贴 https 链接

**Windows（Outlook 新版本）**

1. Outlook 日历视图 → 添加日历 → 从 Internet 订阅 → 粘贴 https 链接

**Outlook 经典版 / Windows 日历**

经典 Outlook 不支持 URL 订阅，请使用方式二下载 ICS 文件导入。

### 方式二：下载文件导入（一次性）

**缺点**：后续赛程更新需重新下载导入。

| 文件 | 语言 | 场次 |
|------|------|------|
| [2026世界杯赛程_简体中文.ics](./2026世界杯赛程_简体中文.ics) | 简体中文 | 104 |
| [2026世界盃賽程_繁體中文.ics](./2026世界盃賽程_繁體中文.ics) | 繁體中文 | 104 |
| [2026_FIFA_World_Cup_Schedule_EN.ics](./2026_FIFA_World_Cup_Schedule_EN.ics) | English | 104 |

**iOS / macOS**：下载文件 → 双击打开 → 系统日历弹出导入提示 → 选择目标日历 → 完成。

**Windows（Outlook 经典版）**：Outlook → 文件 → 打开和导出 → 导入/导出 → 导入 iCalendar (.ics) → 选择文件 → 导入。

**Android**：下载文件 → 文件管理器打开 → 选择系统日历或 Google 日历导入。

### 关键场次

| 比赛 | 北京时间 | 阶段 |
|------|---------|------|
| 墨西哥 vs 南非 | 6月12日 03:00 | A组（揭幕战） |
| 巴西 vs 摩洛哥 | 6月14日 06:00 | C组 |
| 阿根廷 vs 阿尔及利亚 | 6月17日 09:00 | J组 |
| 英格兰 vs 克罗地亚 | 6月18日 04:00 | L组 |
| 葡萄牙 vs 民主刚果 | 6月18日 01:00 | K组 |
| 挪威 vs 法国 | 6月27日 03:00 | I组 |
| 决赛 | 7月20日 03:00 | 决赛 |

### 说明

- 北京时间为 UTC+8，日历文件使用 UTC 时间，导入后设备自动转换。
- 淘汰赛对阵为占位符（如"A组第1"、"Match 73胜者"），待小组赛结束后更新。
- 数据来源：FIFA、Soccerway。

---

## 繁體中文

[:arrow_up: 回到顶部](#2026世界杯赛程日历--2026世界盃賽程日曆--2026-fifa-world-cup-calendar) | [简体中文](#简体中文) | [English](#english)

### 概述

- **賽事**：2026 年美加墨世界盃
- **日期**：2026 年 6 月 11 日 – 7 月 19 日
- **參賽隊伍**：48 支（首次擴軍）
- **總場次**：104 場
- **賽制**：12 個小組 → 32 強 → 16 強 → 8 強 → 半決賽 → 決賽

### 方式一：日曆訂閱（推薦）

**優點**：後續淘汰賽對陣更新後，所有訂閱者自動同步，無需重新匯入。

於 Safari 或 Chrome 中點擊以下連結，iOS / macOS 會自動喚起行事曆應用：

| 語言 | 一鍵訂閱（Safari 點擊） | 手動訂閱連結（複製貼上） |
|------|----------------------|------------------------|
| 简体中文 | <a href="webcal://ztow.github.io/worldcup-2026-calendar/2026%E4%B8%96%E7%95%8C%E6%9D%AF%E8%B5%9B%E7%A8%8B_%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87.ics">點擊訂閱</a> | `https://ztow.github.io/worldcup-2026-calendar/2026世界杯赛程_简体中文.ics` |
| 繁體中文 | <a href="webcal://ztow.github.io/worldcup-2026-calendar/2026%E4%B8%96%E7%95%8C%E7%9B%83%E8%B3%BD%E7%A8%8B_%E7%B9%81%E9%AB%94%E4%B8%AD%E6%96%87.ics">點擊訂閱</a> | `https://ztow.github.io/worldcup-2026-calendar/2026世界盃賽程_繁體中文.ics` |
| English | <a href="webcal://ztow.github.io/worldcup-2026-calendar/2026_FIFA_World_Cup_Schedule_EN.ics">Subscribe</a> | `https://ztow.github.io/worldcup-2026-calendar/2026_FIFA_World_Cup_Schedule_EN.ics` |

> 如果一鍵訂閱無反應，請用 Safari 開啟此頁面重試，或複製右側連結手動新增到行事曆。

#### 各平台詳細步驟

**iPhone / iPad（iOS）**

1. 用 **Safari** 開啟本頁面，點擊上方的「點擊訂閱」
2. 或：設定 → App → 行事曆 → 帳號 → 加入帳號 → 其他 → 加入已訂閱的行事曆 → 貼上 https 連結

**Mac（macOS）**

1. 行事曆 App → 選單列：檔案 → 新增行事曆訂閱 → 貼上 https 連結 → 訂閱

**Android**

1. 用手機瀏覽器開啟 [Google 日曆網頁版](https://calendar.google.com)（需桌面模式）
2. 左側「其他日曆」→ `+` → 透過網址新增 → 貼上 https 連結

**Windows（Outlook 新版本）**

1. Outlook 日曆檢視 → 新增日曆 → 從 Internet 訂閱 → 貼上 https 連結

### 方式二：下載檔案匯入（一次性）

| 檔案 | 語言 | 場次 |
|------|------|------|
| [2026世界杯赛程_简体中文.ics](./2026世界杯赛程_简体中文.ics) | 簡體中文 | 104 |
| [2026世界盃賽程_繁體中文.ics](./2026世界盃賽程_繁體中文.ics) | 繁體中文 | 104 |
| [2026_FIFA_World_Cup_Schedule_EN.ics](./2026_FIFA_World_Cup_Schedule_EN.ics) | English | 104 |

**iOS / macOS**：下載檔案 → 雙擊開啟 → 系統行事曆彈出匯入提示 → 選擇目標行事曆 → 完成。

**Windows（Outlook 經典版）**：Outlook → 檔案 → 開啟和匯出 → 匯入/匯出 → 匯入 iCalendar (.ics) → 選擇檔案 → 匯入。

**Android**：下載檔案 → 檔案管理員開啟 → 選擇系統行事曆或 Google 日曆匯入。

### 重點場次

| 比賽 | 北京時間 | 階段 |
|------|---------|------|
| 墨西哥 vs 南非 | 6月12日 03:00 | A組（揭幕戰） |
| 巴西 vs 摩洛哥 | 6月14日 06:00 | C組 |
| 阿根廷 vs 阿爾及利亞 | 6月17日 09:00 | J組 |
| 英格蘭 vs 克羅地亞 | 6月18日 04:00 | L組 |
| 葡萄牙 vs 民主剛果 | 6月18日 01:00 | K組 |
| 挪威 vs 法國 | 6月27日 03:00 | I組 |
| 決賽 | 7月20日 03:00 | 決賽 |

---

## English

[:arrow_up: Back to Top](#2026世界杯赛程日历--2026世界盃賽程日曆--2026-fifa-world-cup-calendar) | [简体中文](#简体中文) | [繁體中文](#繁體中文)

### Overview

- **Tournament**: 2026 FIFA World Cup (USA / Canada / Mexico)
- **Dates**: June 11 – July 19, 2026
- **Teams**: 48 (expanded from 32)
- **Total matches**: 104
- **Format**: 12 groups → Round of 32 → Round of 16 → Quarter-Finals → Semi-Finals → Final

### Method 1: Calendar Subscription (Recommended)

**Why subscribe?** When knockout matchups are updated after the group stage, all subscribers get changes automatically.

**iOS / macOS users**: Click a link below in Safari to subscribe directly in Calendar.

| Language | One-Click (Safari) | Manual Subscribe URL (copy & paste) |
|----------|-------------------|--------------------------------------|
| 简体中文 | <a href="webcal://ztow.github.io/worldcup-2026-calendar/2026%E4%B8%96%E7%95%8C%E6%9D%AF%E8%B5%9B%E7%A8%8B_%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87.ics">Subscribe</a> | `https://ztow.github.io/worldcup-2026-calendar/2026世界杯赛程_简体中文.ics` |
| 繁體中文 | <a href="webcal://ztow.github.io/worldcup-2026-calendar/2026%E4%B8%96%E7%95%8C%E7%9B%83%E8%B3%BD%E7%A8%8B_%E7%B9%81%E9%AB%94%E4%B8%AD%E6%96%87.ics">Subscribe</a> | `https://ztow.github.io/worldcup-2026-calendar/2026世界盃賽程_繁體中文.ics` |
| English | <a href="webcal://ztow.github.io/worldcup-2026-calendar/2026_FIFA_World_Cup_Schedule_EN.ics">Subscribe</a> | `https://ztow.github.io/worldcup-2026-calendar/2026_FIFA_World_Cup_Schedule_EN.ics` |

> If one-click doesn't work, open this page in Safari and try again, or copy the URL on the right to add manually.

#### Platform Instructions

**iPhone / iPad (iOS)**

1. Open this page in **Safari**, tap the "Subscribe" link above
2. Or: Settings → Apps → Calendar → Accounts → Add Account → Other → Add Subscribed Calendar → paste the https URL

**Mac (macOS)**

1. Calendar.app → File → New Calendar Subscription → paste the https URL → Subscribe

**Android**

1. Open [Google Calendar](https://calendar.google.com) on your phone browser (desktop mode)
2. Left sidebar → Other calendars → `+` → From URL → paste the https URL

**Windows (New Outlook)**

1. Outlook Calendar view → Add calendar → Subscribe from web → paste the https URL

**Classic Outlook / Windows Calendar** does not support subscribe-from-URL. Use Method 2 below.

### Method 2: Download & Import (One-Time)

**Limitation**: re-download needed for schedule updates.

| File | Language | Events |
|------|----------|--------|
| [2026世界杯赛程_简体中文.ics](./2026世界杯赛程_简体中文.ics) | 简体中文 | 104 |
| [2026世界盃賽程_繁體中文.ics](./2026世界盃賽程_繁體中文.ics) | 繁體中文 | 104 |
| [2026_FIFA_World_Cup_Schedule_EN.ics](./2026_FIFA_World_Cup_Schedule_EN.ics) | English | 104 |

**iOS / macOS**: Download → double-click → Calendar prompts import → select target calendar → done.

**Windows (Classic Outlook)**: Outlook → File → Open & Export → Import/Export → Import iCalendar (.ics) → choose file → import.

**Android**: Download → open with file manager → choose system Calendar or Google Calendar to import.

### Key Matches

| Match | Date (UTC) | Stage |
|-------|-----------|-------|
| Mexico vs South Africa | Jun 11, 19:00 | Group A (Opening Match) |
| Brazil vs Morocco | Jun 13, 22:00 | Group C |
| Argentina vs Algeria | Jun 17, 01:00 | Group J |
| England vs Croatia | Jun 17, 20:00 | Group L |
| Portugal vs DR Congo | Jun 17, 17:00 | Group K |
| Norway vs France | Jun 26, 19:00 | Group I |
| Final | Jul 19, 19:00 | Final |

### Notes

- All times are UTC. Your device converts to local timezone automatically.
- Knockout-stage opponents are placeholders (e.g., "Group A Winner", "Match 73 Winner") until group stage concludes.
- Data sourced from FIFA and Soccerway.

---

## Schedule at a Glance / 赛程一览

| Stage / 阶段 | Date Range / 日期 | Matches / 场次 |
|---|---|---|
| Group Stage / 小组赛 | Jun 11 – Jun 28 | 72 |
| Round of 32 / 1/16决赛 | Jun 28 – Jul 4 | 16 |
| Round of 16 / 1/8决赛 | Jul 4 – Jul 7 | 8 |
| Quarter-Finals / 1/4决赛 | Jul 9 – Jul 12 | 4 |
| Semi-Finals / 半决赛 | Jul 14 – Jul 15 | 2 |
| Third-Place / 三四名决赛 | Jul 18 | 1 |
| Final / 决赛 | Jul 19 | 1 |

## License

This project is provided for personal, non-commercial use. Match data is publicly available from FIFA and Soccerway. ICS files are free to use and share.

## Contributing

Match results will become available after each game concludes. PRs to fill in knockout-stage opponents are welcome once group matches are completed.
